  ...
  with Serializable
{
  ...
  //updated by HPL, for shortcircuit  the combiner for aggregation
  def scombineByKey[C](shortcircuitCombiner: Boolean,
                      createCombiner: V => C,
                      mergeValue: (C, V) => C,
                      mergeCombiners: (C, C) => C,
                      partitioner: Partitioner,
    ...
    val aggregator = new Aggregator[K, V, C](createCombiner, mergeValue, mergeCombiners)
    //updated by HPL, for chortcircuit the combiner for aggregation
  ...
  def groupByKey(partitioner: Partitioner): RDD[(K, Iterable[V])] = {
    ...
    val bufs = scombineByKey[CompactBuffer[V]](
      shortcircuitCombiner,
