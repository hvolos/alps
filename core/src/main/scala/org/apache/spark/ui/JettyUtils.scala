...
private[spark] object JettyUtils extends Logging {
  ...
  def startJettyServer(
    ...
    def connect(currentPort: Int): (Server, Int) = {
      ...
      // HPL: to accomodate large cores for large multicore machines
      val min = 1 
      val MAX_NUMBER_FOR_HTTP_SERVER_THREADS=10 
      pool.setMinThreads(min)
      pool.setMaxThreads(MAX_NUMBER_FOR_HTTP_SERVER_THREADS)

      server.setThreadPool(pool)
      server.setHandler(collection)

      // HPL: to accomodate smaller number of acceptors 
      val connectors=server.getConnectors()
      val MAX_NUMBER_FOR_HTTP_SERVER_ACCEPTOR=3
      if (connectors(0).isInstanceOf[AbstractConnector]) {
          val cconnector = connectors(0).asInstanceOf[AbstractConnector]          
          cconnector.setAcceptors(MAX_NUMBER_FOR_HTTP_SERVER_ACCEPTOR)
      }
