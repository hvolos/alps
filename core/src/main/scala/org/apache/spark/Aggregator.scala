...
case class Aggregator[K, V, C] (
  ...
  //modified by HPL, for short-circuit the combiner, as C++ shuffle engine returns
  //(K, Seq(V)) already
  var shortcircuitedCombiner_ = false

  //set it at the groupby RDD
  def shortcircuitCombiner(sval: Boolean): Unit = {
    shortcircuitedCombiner_ = sval
  }

  def shortcircuitCombiner: Boolean = shortcircuitedCombiner_

  //modified by HPL, to C++ shuffle engine
  def combineMultiValuesByKey(iter: Iterator[_ <: Product2[K, Seq[V]]],
                         context: TaskContext): Iterator[(K, C)] = {
    if (shortcircuitedCombiner_) {
      iter.asInstanceOf[Iterator[(K, C)]]
    }
    else {
