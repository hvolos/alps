...
object SparkEnv extends Logging {
  ...
  private def create(
    ...
    // Added by HPL, to introduce shared-memory based shuffle
    val shortShuffleMgrNames = Map(
      "hash" -> "org.apache.spark.shuffle.hash.HashShuffleManager",
      "sort" -> "org.apache.spark.shuffle.sort.SortShuffleManager",
      "shm"  -> "org.apache.spark.shuffle.shm.ShmShuffleManager")
    // The default is sort. for shm, we will need: spark.shuffle.manager -> shm
    val shuffleMgrName = conf.get("spark.shuffle.manager", "sort")
    val shuffleMgrClass = shortShuffleMgrNames.getOrElse(shuffleMgrName.toLowerCase, shuffleMgrName)
    val shuffleManager = instantiateClass[ShuffleManager](shuffleMgrClass)
    //for shuffle manager, we need to differntiate is local or not.
    if (shuffleMgrName == "shm") {
       val shmShuffleManager =
             shuffleManager.asInstanceOf[org.apache.spark.shuffle.shm.ShmShuffleManager]
       shmShuffleManager.initialize(isLocal)
    }
