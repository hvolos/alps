...
class  TestShmShuffleMultiThreadedOperations extends FunSuite with Matchers with LocalSparkContext with Logging {
  ...
  //to configure log4j properties to follow what are specified in log4j.properties
   ...
   ignore("groupBy with two partitions") {
  ...
  ignore ("groupByKey with duplicates") {
    val conf = new SparkConf(false)
    //to supress a null-pointer from running the test case.
    conf.set("spark.shuffle.manager", "shm")
    conf.set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
 ...
 ignore ("fullOuterJoin") {
  ...
  ignore ("lookup with partitioner") {
    val conf = new SparkConf(false)
    //to supress a null-pointer from running the test case.
    conf.set("spark.shuffle.manager", "shm")
    conf.set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
