...
#ifndef MAPSHUFFLESTORE_WITH_LONG_KEYS_H_
#define MAPSHUFFLESTORE_WITH_LONG_KEYS_H_
...
struct LongKeyWithValueTracker {
  long key;
  int partition;
  PositionInExtensibleByteBuffer value_tracker;

  LongKeyWithValueTracker (long k, int p,  const PositionInExtensibleByteBuffer& tracker) 
    : key (k),  partition (p), value_tracker(tracker) {
 
  };
};

class  MapShuffleStoreWithLongKey: public GenericMapShuffleStore {
 private:
     //change from vector to become dynamic array
     //vector <LongKeyWithValueTracker> keys;
     LongKeyWithValueTracker *keys;    
 ...
 public: 
     ...
     MapShuffleStoreWithLongKey (int bufsize, int mId, bool ordering);
     //please refer to C++ 11 reference book page. 194                          
     MapShuffleStoreWithLongKey(int mId, bool ordering);

     //Item 7: declare a virtual destructor if and only if the class contains at least one virtual function
     virtual ~MapShuffleStoreWithLongKey () {
     ...
     //for testing purpose.
     LongKeyWithValueTracker* getKeys() {
       return keys;
     }

     void storeKVPairsWithLongKeys (unsigned char *byteHolder, int voffsets[], 
...
#endif  /*MAPSHUFFLESTORE_WITH_LONG_KEYS_H_*/
