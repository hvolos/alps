...
#ifndef  _HASH_MAP_BASED_REDUCE_CHANNEL_WITH_LONG_KEY_H_
#define  _HASH_MAP_BASED_REDUCE_CHANNEL_WITH_LONG_KEY_H_
 ...
 *
*/
class  HashMapReduceChannelWithLongKeys: public GenericReduceChannel{
private:   
	int  kvalueCursor;
...
public: 
	...
	HashMapReduceChannelWithLongKeys (MapBucket &sourceBucket, int rId, int rPartitions,
	...
	~HashMapReduceChannelWithLongKeys() {
		//do nothing. 
	}
...
class HashMapReduceEngineWithLongKeys {
private:
	vector <HashMapReduceChannelWithLongKeys>  hashMapReduceChannels;
	int reduceId;
...
private:
	...
	unordered_map<long, int> hashMergeTable; 
	...
	KeyWithFixedLength::HashMapValueLinkingWithSameKey  valueLinkList;
        ...
        unordered_map<long, int>::iterator hashMapIterator;
public: 

	//passed in: the reducer id and the total number of the partitions for the reduce side.
	HashMapReduceEngineWithLongKeys(int rId, int rPartitions, ExtensibleByteBuffers *bufMgr) :
	...
	void addHashMapReduceChannel(MapBucket &mapBucket) {
	    HashMapReduceChannelWithLongKeys  channel(mapBucket, 
...
#endif /*_HASH_MAP_BASED_REDUCE_CHANNEL_WITH_LONG_KEY_H_*/
